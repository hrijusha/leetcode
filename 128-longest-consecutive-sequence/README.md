...
Space Complexity: O(log N) While your algorithm doesn't explicitly allocate any new data structures (like arrays or hash maps), the space complexity is not strictly O(1) because of the sorting function.Sorting overhead: Java's Arrays.sort() for primitive types is an in-place sort, but the recursive nature of the Dual-Pivot Quicksort algorithm requires auxiliary space on the call stack. This takes O(log N). 
